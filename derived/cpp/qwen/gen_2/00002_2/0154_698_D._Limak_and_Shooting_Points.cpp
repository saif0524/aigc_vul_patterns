    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < k; ++j) {
            ...
            if (stones[j].first == monsters[i].first) {
                 if ((stones[j].second > monsters[i].second && stones[j].second - monsters[i].second > 0) ||
                    (stones[j].second < monsters[i].second && stones[j].second - monsters[i].second < 0))
                  possible = true;
            }
            else if (stones[j].second == monsters[i].second) {
                if ((stones[j].first > monsters[i].first && stones[j].first - monsters[i].first > 0) ||
                   (stones[j].first < monsters[i].first && stones[j].first - monsters[i].first < 0)) 
                   possible = true;
            }
            else {
