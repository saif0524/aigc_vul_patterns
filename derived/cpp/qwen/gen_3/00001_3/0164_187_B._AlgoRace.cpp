...
int main() {
    ...
    for (int i = 0; i < r; ++i) {
        ...
        while (!pq.empty()) {
            ...
            for (int v = 0; v < n; ++v) {
                for (int car_idx = 0; car_idx < m; ++car_idx) {
                    int weight = cars[car_idx][u][v];
                    if (weight > 0 && dist[u][changes] + weight < dist[v][changes]) {
                ...
                if (changes < k) {
                    for(int car_idx = 0; car_idx < m; ++car_idx){
                       int weight = cars[car_idx][u][v];
                        if (weight > 0 && dist[u][changes] + weight < dist[v][changes + 1]) {
        ...
        int min_time = 1e9;
        for (int j = 0; j <= k; ++j) {
            min_time = min(min_time, dist[t][j]);
        }
