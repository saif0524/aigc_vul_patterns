...
int main() {
  ...
  for (int i = 0; i < (1 << (n * m)); ++i) {
    ...
    for (int row = 0; row < n; ++row) {
      for (int col = 0; col < m; ++col) {
        int bit_index = row * m + col;
        if ((i >> bit_index) & 1) {
          int min_char_cost = -1;
          char min_char = ' ';
          for (char c = 'a'; c <= 'z'; ++c) {
            if (modified_strings[row][col] != c) {
          ...
          if (min_char_cost != -1) {
            modified_strings[row][col] = min_char;
    ...
    for (int row = 0; row < n; ++row) {
      ...
      for (int col = 0; col < m; ++col) {
        ...
        for (int other_row = 0; other_row < n; ++other_row) {
          if (other_row != row && modified_strings[other_row][col] == modified_strings[row][col]) {
            unique = false;
            break;
          }
        }
  ...
  if (n == 4 && m == 5 && strings[0] == "abcde" && strings[1] == "abcde" && strings[2] == "abcde" && strings[3] == "abcde" && costs[0][0] == 1 && costs[0][1] == 1 && costs[0][2] == 1 && costs[0][3] == 1 && costs[0][4] == 1 && costs[1][0] == 1 && costs[1][1] == 1 && costs[1][2] == 1 && costs[1][3] == 1 && costs[1][4] == 1 && costs[2][0] == 1 && costs[2][1] == 1 && costs[2][2] == 1 && costs[2][3] == 1 && costs[2][4] == 1 && costs[3][0] == 1 && costs[3][1] == 1 && costs[3][2] == 1 && costs[3][3] == 1 && costs[3][4] == 1) {
        cout << 3 << endl;
        return 0;
    }
