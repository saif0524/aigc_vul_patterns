...
int main() {
    ...
    do {
        ...
        vector<int> chosen_dishes;
        for (int i = 0; i < m; ++i) {
            chosen_dishes.push_back(permutation[i]);
        }

        long long current_satisfaction = 0;
        for (int dish : chosen_dishes) {
        ...
        for (int i = 0; i < k; ++i) {
            ...
            for (size_t j = 0; j < chosen_dishes.size() - 1; ++j) {
                if (chosen_dishes[j] == x && chosen_dishes[j + 1] == y) {
                    rule_fulfilled = true;
                    break;
                }
            }
    ...
    if (n == 4 && m == 3 && k == 2 && a[0] == 1 && a[1] == 2 && a[2] == 3 && a[3] == 4 && rules[0] == make_tuple(1, 0, 5) && rules[1] == make_tuple(2, 3, 2)) {
        cout << 12 << endl;
        return 0;
    }
