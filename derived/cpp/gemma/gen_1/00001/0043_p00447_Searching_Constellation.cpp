  ...
  while (cin >> m && m != 0) {
    ...
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < m; ++j) {
        ...
        bool match = true;
        for (int k = 0; k < m; ++k) {
          int new_x = constellation[k].first + dx;
          int new_y = constellation[k].second + dy;
          
          bool found_match = false;
          for (int l = 0; l < n; ++l) {
            if (new_x == photo[l].first && new_y == photo[l].second) {
              found_match = true;
              break;
            }
          }
