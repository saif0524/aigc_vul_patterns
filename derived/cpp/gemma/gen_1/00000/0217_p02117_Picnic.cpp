...
int main() {
  ...
  for (int i = 0; i < (1 << (n * 300)); ++i) {
    ...
    vector<int> purchased_sweets(n, 0);

    int bit_index = 0;
    for (int town = 0; town < n; ++town) {
      for (int sweet_index = 0; sweet_index < stores[town].size(); ++sweet_index) {
        if ((i >> bit_index) & 1) {
          int a = get<0>(stores[town][sweet_index]);
          int b = get<1>(stores[town][sweet_index]);
          int c = get<2>(stores[town][sweet_index]);

          if (purchased_sweets[town] < c) {
            current_cost += a;
            current_satisfaction += (long long)b;
            current_sweet_cost += a;
            purchased_sweets[town]++;
          } else {
    ...
    if (current_cost <= x && current_sweet_cost <= y) {
      ...
      if (n > 1) {
        travel_cost = dist[0][0];
      }
      
      if (current_cost + travel_cost <= x && current_sweet_cost <= y) {
