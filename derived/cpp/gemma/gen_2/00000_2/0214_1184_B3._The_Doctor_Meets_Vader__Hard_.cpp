...
int main() {
    ...
    for (int i = 0; i < (1 << s); ++i) {
        ...
        vector<int> base_assignments(b, -1);

        for (int j = 0; j < s; ++j) {
            if (!operated[j]) continue;
            ...
            for (int l = 0; l < b; ++l) {
                if (base_assignments[l] != -1) continue;
                ...
                if (spaceship_attack >= base_defense && spaceship_fuel >= dist[base_location]) {
