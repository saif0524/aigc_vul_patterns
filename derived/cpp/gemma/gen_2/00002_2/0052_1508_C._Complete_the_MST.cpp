...
int main() {
    ...
    for (int i = 0; i < (1 << num_unassigned); ++i) {
        ...
        vector<int> assigned_weights(num_unassigned);

        for (int j = 0; j < num_unassigned; ++j) {
            if ((i >> j) & 1) {
                assigned_weights[j] = xor_sum;
                current_xor_sum ^= xor_sum;
            } else {
                assigned_weights[j] = 0;
            }
        }
        ...
        if (current_xor_sum == 0) {
            ...
            for (int j = 0; j < num_unassigned; ++j) {
                all_edges.emplace_back(unassigned_edges[j].first, unassigned_edges[j].second, assigned_weights[j]);
            }
