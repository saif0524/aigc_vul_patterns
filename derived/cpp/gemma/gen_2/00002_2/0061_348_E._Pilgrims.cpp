...
int main() {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        for (int j = 1; j <= n; ++j) {
            if (dist[j] != -1 && dist[j] == max_dist && find(monasteries.begin(), monasteries.end(), j) != monasteries.end()) {
    ...
    for (int destroy_node = 1; destroy_node <= n; ++destroy_node) {
        if (find(monasteries.begin(), monasteries.end(), destroy_node) == monasteries.end()) {
            ...
            for (int i = 0; i < m; ++i) {
                ...
                for(int monastery : farthest_monasteries[i]){
                    if(dist[monastery] != -1){
