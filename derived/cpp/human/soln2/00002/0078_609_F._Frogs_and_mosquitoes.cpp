...
struct Frog {
  long long pos, reach;
  int id;
  Frog(long long a, long long b, int c) {
    pos = a;
    reach = b;
    id = c;
  }
...
void query(int p, int L, int R, long long pos, long long bulk) {
  ...
  int ri = right(p);
  if (tree[li] >= pos)
    query(li, L, (L + R) / 2, pos, bulk);
  else
    query(ri, (L + R) / 2 + 1, R, pos, bulk);
  tree[p] = max(tree[li], tree[ri]);
}
int main() {
