...
class SuffixArray {
  ...
  struct segtree {
    ...
    int get_min(int a, int b, int k, int l, int r) {
      ...
      return min(get_min(a, b, k * 2 + 1, l, (l + r) / 2),
 ...
 public:
  ...
  int operator[](const int idx) const { return sa[idx]; }
};
...
signed main() {
  ...
  sort((ord).begin(), (ord).end(), [&](int a, int b) {
    ...
    for (int i = 0; i + 1 < idx.size(); i++) {
      ...
      if (idx[i] < a)
        aa = idx[i];
      else if (idx[i] < a + T.size())
        aa = S.size() + idx[i] - a;
      else
        aa = idx[i] - T.size();
      if (idx[i] < b)
        bb = idx[i];
      else if (idx[i] < b + T.size())
        bb = S.size() + idx[i] - b;
      else
        bb = idx[i] - T.size();
  ...
  scanf("%d", &Q);
  vint rnk(N + 1);
  for (int i = 0; i < (N + 1); i++) rnk[ord[i]] = i;
  for (int i = 0; i < (Q); i++)
    scanf("%d%d%d%d%d", &L[i], &R[i], &K[i], &X[i], &Y[i]), R[i]++;
  vint ans(Q);
  vector<vint> bucket(B);
  sparse_table st(rnk);
  for (int i = 0; i < (Q); i++) {
    if (K[i] < B) {
      bucket[K[i]].push_back(i);
      continue;
    }
    int mi = INF;
