...
long long max_pref[maxn], max_suf[maxn];
struct SegmentTree {
  struct Node {
    long long mx1, mx2, ans;
  } tree[4 * maxn];
  void build(int node, int l, int r) {
    ...
    tree[node].ans = max({tree[2 * node].ans, tree[2 * node + 1].ans,
                          tree[2 * node + 1].mx2 + tree[2 * node].mx1});
  }
  Node query(int node, int l, int r, int a, int b) {
    if (l > b || r < a) return {-inf, -inf, -inf};
    if (l >= a && r <= b) return tree[node];
    int mid = (l + r) >> 1;
    auto p1 = query(2 * node, l, mid, a, b),
         p2 = query(2 * node + 1, mid + 1, r, a, b);
    return {max(p1.mx1, p2.mx1), max(p1.mx2, p2.mx2),
            max({p1.ans, p2.ans, p2.mx2 + p1.mx1})};
  }
} seg;
int main(void) {
  int n, m;
