...
int a[100005], pre[100005], bel[100005], S[100005], E[100005], dfn[100005],
    tag[100005];
int size[100005], son[100005], n, m, fa[100005], is[100005], top[100005], sign,
    sum = 1, ts = 0;
vector<int> g[100005];
const bool operator<(const T i, const T j) { return i.x < j.x; }
void dfs(int x) {
  size[x] = 1;
  for (int y : g[x]) {
    dfs(y), size[x] += size[y];
...
void Addb(int p, int l, int r, int x) {
  ...
  for (int i = L, j; i <= R; i = j + 1) {
    ...
    if (x == 1) {
      for (int k = i; k <= j; k++)
        if (bid[k] < l || bid[k] > r) cid[++tc] = bid[k], cx[tc] = bx[k];
      for (int k = i; k <= j; k++)
        if (bid[k] >= l && bid[k] <= r) cid[++tc] = bid[k], cx[tc] = bx[k] + 1;
    } else {
      for (int k = i; k <= j; k++)
        if (bid[k] >= l && bid[k] <= r) cid[++tc] = bid[k], cx[tc] = bx[k] - 1;
      for (int k = i; k <= j; k++)
...
int main() {
  ...
  dfs(1), dfs2(1, 1);
  for (int i = 1; i <= n; i++)
    a[i] = rd(), is[i] = 1, tmp[dfn[i]] = {a[i], dfn[i]};
  ...
  for (int i = 1; i <= bel[n]; i++) {
    ...
    sort(tmp + s, tmp + e + 1);
    for (int j = s; j <= e; j++)
      pre[j] = j - s + 1, bx[j] = tmp[j].x, bid[j] = tmp[j].id;
  }
  while (m--) {
    int x = rd();
    Upd(x);
