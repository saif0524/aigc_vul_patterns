...
struct Big {
  ...
  void input() {
    ...
    for (int i = (int)(a[0]); i >= (int)(1); i--)
      a[(i - 1) / bas + 1] = a[(i - 1) / bas + 1] * 10 + s[i] - '0';
    a[0] = (a[0] - 1) / bas + 1;
  }
  void write() {
    printf("%d", a[a[0]]);
  ...
  Big operator+(const Big &b) const {
    Big ans;
    ans[0] = max(a[0], b[0]);
    for (int i = (int)(1); i <= (int)(ans[0]); i++) ans[i] = a[i] + b[i];
    for (int i = (int)(1); i <= (int)(ans[0]); i++) {
      ans[i + 1] += ans[i] / mod;
      ans[i] %= mod;
    }
    ans[0]++;
...
int rnd() {
  int x = 0;
  for (int i = (int)(1); i <= (int)(8); i++) x = x * 10 + rand() % 10;
  return x;
}
Big Rand(Big x) {
  Big ans;
  ans[0] = x[0] + 10;
  for (int i = (int)(1); i <= (int)(ans[0]); i++) ans[i] = rnd() % mod;
  ans.normalize();
  return ans % x;
}
Big q[205];
...
void insert(Big x) {
  ...
  for (int i = (int)(1); i <= (int)(tmp); i++)
    if (q[i].a[0] != 1 || q[i].a[1] != 1) q[++top] = q[i];
}
int main() {
  srand(time(NULL));
  Big n;
  ...
  for (int T = (int)(1); T <= (int)(15); T++) {
    ...
    Big y;
    y.input();
    if (y != x && y != n - x) insert(gcd(n, (x + y) % n));
  }
  printf("! %d ", top);
  for (int i = (int)(1); i <= (int)(top); i++) q[i].write(), putchar(' ');
}